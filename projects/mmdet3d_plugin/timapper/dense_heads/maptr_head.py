...
class MapTRHead(DETRHead):
    ...
    def __init__(self,
                 *args,
                 ...
                 oversampled_num_pts_per_gt_vec=100,
                 ...
                 instance_level_class=True,
                 maptr_baseline=False,
                 query_fusion=False,
                 scalar=2,
        ...
        self.instance_level_class=instance_level_class
        self.maptr_baseline = maptr_baseline
        ...
        self.oversampled_num_pts_per_gt_vec = oversampled_num_pts_per_gt_vec
    ...
    def _init_layers(self):
        ...
        cls_branch = []
        # if not self.maptr_baseline and self.instance_level_class:
    ...
    def forward(self, 
        ...
        if self.query_embed_type == 'all_pts':
            object_query_embeds = self.query_embedding.weight.to(dtype).reshape(self.num_vec,self.num_pts_per_vec,-1)
        elif self.query_embed_type == 'instance_pts':
            pts_embeds = self.pts_embedding.weight.unsqueeze(0)
            instance_embeds = self.instance_embedding.weight.unsqueeze(1)
            object_query_embeds = (pts_embeds + instance_embeds).to(dtype)
        elif self.query_embed_type == 'hybrid_pts':
            pts_embeds = self.pts_embedding.weight.reshape(self.num_vec, self.num_pts_per_vec, -1)
            instance_embeds = self.instance_embedding.weight.unsqueeze(1).repeat(1, self.num_pts_per_vec, 1)
            object_query_embeds = (pts_embeds + instance_embeds).to(dtype)
        
        if self.query_fusion=='self_attn':
            ...
            object_query_embeds = object_query_embeds.permute(1,0,2)
            ...
            object_query_embeds = object_query_embeds.permute(1,0,2)
        elif self.query_fusion=='mean':
            object_query_embeds = object_query_embeds + object_query_embeds.mean(dim=1).unsqueeze(1).repeat(1,self.num_pts_per_vec,1)


        object_query_embeds = object_query_embeds.flatten(0, 1)
    ...
    def generate_even_sample_pts_torch(self,raw_points,num_sampled_points):
        # raw_points shape: b, num_vec*num_pts_per_vec, 2
        num_vec = self.num_vec
        num_pts_per_vec = self.num_pts_per_vec
        bs = raw_points.shape[0]
        raw_points = raw_points.reshape(bs,num_vec,num_pts_per_vec,2)
        # 
        delta_pts = raw_points - torch.cat([raw_points[:,:,0:1],raw_points[:,:,:-1]],dim=2)
        distances = torch.pow(torch.pow(delta_pts[:,:,:,0],2) + torch.pow(delta_pts[:,:,:,1],2),0.5)

        sum_length = torch.sum(distances,dim=-1)
        delta_length = sum_length / (num_sampled_points-1)

        range_list = torch.arange(0,num_sampled_points).expand(bs,num_vec,-1).to(delta_length.device)
        sampled_points_length = range_list*delta_length[:,:,None]
        raw_points_length = torch.cumsum(distances,dim=-1)
        sampled_length_compare_raw_length = (sampled_points_length.unsqueeze(3)>=raw_points_length.unsqueeze(2))
        sampled_points_index = torch.sum(sampled_length_compare_raw_length,dim=3).clamp(0,num_pts_per_vec-1)

        sampled_points_current_segment_length = sampled_points_length-\
                                        torch.gather(raw_points_length,2,sampled_points_index-1)
        current_segment_length = torch.gather(raw_points_length,2,sampled_points_index)-\
                                torch.gather(raw_points_length,2,sampled_points_index-1)
        alpha = sampled_points_current_segment_length/current_segment_length
        alpha = alpha.unsqueeze(3).repeat(1,1,1,2).detach()
        sampled_points_index = sampled_points_index.detach()
        sampled_points_index = sampled_points_index.unsqueeze(3).repeat(1,1,1,2)
        
        sampled_points = alpha*torch.gather(raw_points,2,sampled_points_index)+\
                            (1-alpha)*torch.gather(raw_points,2,sampled_points_index-1)
    ...
    def _get_target_single(self,
        ...
        if not self.use_oversampling:
            ...
            oversampled_assigned_shift = assigned_shift
            oversampled_pts_targets = pts_pred.new_zeros((pts_pred.size(0),
                            gt_oversampled_shifts_pts.size(2), pts_pred.size(2)))
            oversampled_pts_weights = torch.zeros_like(oversampled_pts_targets)
            oversampled_pts_weights[pos_inds] = 1.0
            oversampled_pts_targets[pos_inds] = gt_oversampled_shifts_pts[sampling_result.pos_assigned_gt_inds,oversampled_assigned_shift,:,:]
        
            return (labels, label_weights, bbox_targets, bbox_weights,
                    oversampled_pts_targets, oversampled_pts_weights,
                    pos_inds, neg_inds)
    ...
    def get_targets(self,
                    ...
                    gt_labels_list,
                    gt_shifts_pts_list,
                    gt_oversampled_shifts_pts_list,
        ...
        (labels_list, label_weights_list, bbox_targets_list,
         ...
         pos_inds_list, neg_inds_list) = multi_apply(
            ...
            gt_labels_list, gt_bboxes_list, gt_shifts_pts_list, gt_oversampled_shifts_pts_list, gt_bboxes_ignore_list)
    ...
    def loss_single(self,
        ...
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list,pts_preds_list,
                                           gt_bboxes_list, gt_labels_list,gt_shifts_pts_list,gt_oversampled_shifts_pts_list,
                                           gt_bboxes_ignore_list)
        ...
        if self.use_oversampling:
            pts_preds = self.generate_even_sample_pts_torch(pts_preds, self.oversampled_num_pts_per_gt_vec)
            # num_total_pos = num_total_pos * self.oversampled_num_pts_per_gt_vec / self.num_pts_per_gt_vec
    ...
    def loss(self,
        ...
        # gt_oversampled_shifts_pts_list = [
        #     gt_bboxes.oversampled_shift_fixed_num_sampled_points_v2.to(device) for gt_bboxes in gt_vecs_list]
        ...
        # all_gt_oversampled_shifts_pts_list = [gt_oversampled_shifts_pts_list for _ in range(num_dec_layers)]
        ...
        losses_cls, losses_bbox, losses_iou, losses_pts, losses_dir = multi_apply(
            ...
            all_gt_bboxes_list, all_gt_labels_list,all_gt_shifts_pts_list,all_gt_shifts_pts_list,
            all_gt_bboxes_ignore_list)
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_pts_i, loss_dir_i in zip(losses_cls[:-1],
                                           losses_bbox[:-1],
                                           losses_pts[:-1],
                                           losses_dir[:-1]):
