...
def format_res_gt_by_classes(result_path,
    ...
    if val_vis:
        ...
        for i in np.linspace(0,len(gen_results)-1,dtype=int,num=10):
            if not osp.isdir(osp.join(show_dir, f'MAP_{i}')):
                os.makedirs(osp.join(show_dir, f'MAP_{i}'),exist_ok=True)
            ...
            for line in gen_results[i]['vectors']:
                l = np.array(line['pts'])
                pts_x, pts_y = l[:,0]+pc_range[0],l[:,1]
                plt.plot(pts_x, pts_y, color=colors_plt[line['type']],linewidth=1,alpha=0.8,zorder=-1)
                plt.scatter(pts_x, pts_y, color=colors_plt[line['type']],s=1,alpha=0.8,zorder=-1)

            
            for line in annotations[i]['vectors']:
                l = np.array(line['pts'])
                pts_x, pts_y = l[:,0]+pc_range[3],l[:,1]
                plt.plot(pts_x, pts_y, color=colors_plt[line['type']],linewidth=1,alpha=0.8,zorder=-1)
                plt.scatter(pts_x, pts_y, color=colors_plt[line['type']],s=1,alpha=0.8,zorder=-1)

                    
            plt.imshow(car_img, extent=[-1.2+pc_range[0], 1.2+pc_range[0], -1.5, 1.5],zorder=4)
            plt.imshow(car_img, extent=[-1.2+pc_range[3], 1.2+pc_range[3], -1.5, 1.5],zorder=4)
            map_path = osp.join(show_dir, f'MAP_{i}/main_{temp_epoch_counter}.jpg')
