...
class Graph():
    ...
    def add_e(self,e):
        if e.length==0:
            return
        src, dst = e.boundary.geoms
        v1 = self.add_v(src)
        v2 = self.add_v(dst)
        new_edge = Edge(e,v1,v2,self.edge_counter)
        self.edge_counter += 1
    ...
    def merge(self,edges):
        ...
        if (e1.dst == e2.src and e1.src == e2.dst) or (e1.dst == e2.dst and e1.src == e2.src):
            ...
            self.edges = [e for e in self.edges if e!=e1 and e!=e2]
...
class LiDARInstanceLines(object):
    ...
    def __init__(self, 
        ...
        self.oversampled_fixed_num = oversampled_fixed_num
        self.padding_value = padding_value

        self.instance_list = instance_line_list
        self.type_list = instance_type_list
    ...
    @property
    def get_gt_adjacency(self):
        """
        Get the adjacency matrix of the ground truth line instances
        """
        output_intersection_points = []
        output_overlapping_points = []
        line_geoms = [{'s':self.instance_list[ii],'id':ii} for ii, c in enumerate(self.type_list) if c==0]
        ped_geoms = [{'s':self.instance_list[ii],'id':ii} for ii, c in enumerate(self.type_list) if c==1]
        polygon_geoms = [{'s':self.instance_list[ii],'id':ii} for ii, c in enumerate(self.type_list) if c==2]
        centerline_geoms = [{'s':self.instance_list[ii],'id':ii} for ii, c in enumerate(self.type_list) if c==3]
        # except centerline, other shapes have no inter-class crossing
        for line_geom in line_geoms:
            for geom in ped_geoms+polygon_geoms+centerline_geoms:
                new_point = line_geom['s'].intersection(geom['s'])
                if not new_point.is_empty:
                    if new_point.geom_type == 'MultiPoint':
                        output_overlapping_points.append([line_geom['id'],geom['id'],[list(p.coords)[0] for p in new_point.geoms]])
                    elif new_point.geom_type == 'Point':
                        output_overlapping_points.append([line_geom['id'],geom['id'],list(new_point.coords)])
        for ped_geom in ped_geoms:
            for geom in polygon_geoms+centerline_geoms:
                new_point = ped_geom['s'].intersection(geom['s'])
                if not new_point.is_empty:
                    if new_point.geom_type == 'MultiPoint':
                        output_overlapping_points.append([ped_geom['id'],geom['id'],[list(p.coords)[0] for p in new_point.geoms]])
                    elif new_point.geom_type == 'Point':
                        output_overlapping_points.append([ped_geom['id'],geom['id'],list(new_point.coords)])
        for polygon_geom in polygon_geoms:
            for geom in centerline_geoms:
                new_point = polygon_geom['s'].intersection(geom['s'])
                if not new_point.is_empty:
                    if new_point.geom_type == 'MultiPoint':
                        output_overlapping_points.append([polygon_geom['id'],geom['id'],[list(p.coords)[0] for p in new_point.geoms]])
                    elif new_point.geom_type == 'Point':
                        output_overlapping_points.append([polygon_geom['id'],geom['id'],list(new_point.coords)])
        for center_idx, centerline_geom in enumerate(centerline_geoms[:-1]):
            for geom in centerline_geoms[center_idx+1:]:
                if centerline_geom['id'] == geom['id']:
                    raise Exception('Incorrect centerline matching...')
                new_point = centerline_geom['s'].intersection(geom['s'])
                src, dst = centerline_geom['s'].boundary.geoms
                endpoints_line1 = [list(src.coords)[0],list(dst.coords)[0]]
                src, dst = geom['s'].boundary.geoms
                endpoints_line2 = [list(src.coords)[0],list(dst.coords)[0]]
                if not new_point.is_empty:
                    if new_point.geom_type == 'MultiPoint':
                        new_point = [list(p.coords)[0] for p in new_point.geoms]
                        intersection_point = [p for p in new_point if p in endpoints_line1 or p in endpoints_line2]
                        overlapping_point = [p for p in new_point if not (p in endpoints_line1 or p in endpoints_line2)]
                        if len(intersection_point):
                            output_intersection_points.append([centerline_geom['id'],geom['id'],intersection_point])
                        # # At this stage, we treat multi-overlapping points between two centerlines as noises, which is 
                        # # caused by inaccurate annotation of NuScenes
                        # if len(overlapping_point):
                        #     output_overlapping_points.append([centerline_geom['id'],geom['id'],overlapping_point])
                    elif new_point.geom_type == 'Point':
                        new_point = list(new_point.coords)
                        if new_point[0] in endpoints_line1 or new_point[0] in endpoints_line2:
                            output_intersection_points.append([centerline_geom['id'],geom['id'],new_point])
                        else:
                            output_overlapping_points.append([centerline_geom['id'],geom['id'],new_point])
        # ===================== post-processing connection points for gt_labels =============
        instance_len = len(self.type_list)
        # add one more label as the first element to avoid empty tensor
        gt_adjacent_matrix = np.zeros((instance_len,instance_len))
        # np.fill_diagonal(gt_adjacent_matrix,1)
        gt_adjacent_pts = np.zeros((instance_len,instance_len,2))
        
        # overlapping points
        for p in output_overlapping_points:
            id_1, id_2 = p[0], p[1]
            gt_adjacent_matrix[id_1,id_2] = 1
            # gt_adjacent_pts[id_1,id_2] = np.array(p[-1])

            gt_adjacent_matrix[id_2,id_1] = 1
            # gt_adjacent_pts[id_2,id_1] = np.array(p[-1])

        # intersection points
        for p in output_intersection_points:
            id_1, id_2 = p[0], p[1]
            gt_adjacent_matrix[id_1,id_2] = 1
            # gt_adjacent_pts[id_1,id_2] = np.array(p[-1])
        ...
        return {
                'adjacent_matrix':gt_adjacent_matrix, 
                'adjacent_pts':gt_adjacent_pts
                }
    ...
    def shift_fixed_num_sampled_points_v2(self):
        ...
        assert len(self.instance_list) != 0
        instances_list = []
        for ins_idx, instance in enumerate(self.instance_list):
            distances = np.linspace(0, instance.length, self.fixed_num)
            ...
            if self.ordered and self.type_list[ins_idx] == 3:
                shift_num = 1
    ...
    @property
    def oversampled_shift_fixed_num_sampled_points_v2(self):
        """
        return  [instances_num, num_shifts, fixed_num, 2]
        """
        assert len(self.instance_list) != 0
        instances_list = []
        for instance in self.instance_list:
            distances = np.linspace(0, instance.length, self.oversampled_fixed_num)
            poly_pts = np.array(list(instance.coords))
            start_pts = poly_pts[0]
            end_pts = poly_pts[-1]
            is_poly = np.equal(start_pts, end_pts)
            ...
            pts_num, coords_num = poly_pts.shape
            shift_num = pts_num - 1
            final_shift_num = self.oversampled_fixed_num - 1
            if is_poly:
                pts_to_shift = poly_pts[:-1,:]
                for shift_right_i in range(shift_num):
                    shift_pts = np.roll(pts_to_shift,shift_right_i,axis=0)
            ...
            # if not is_poly:
            if multi_shifts_pts_tensor.shape[0] < final_shift_num:
                padding = torch.full([final_shift_num-multi_shifts_pts_tensor.shape[0],self.oversampled_fixed_num,2], self.padding_value)
                multi_shifts_pts_tensor = torch.cat([multi_shifts_pts_tensor,padding],dim=0)
            instances_list.append(multi_shifts_pts_tensor)
        instances_tensor = torch.stack(instances_list, dim=0)
        instances_tensor = instances_tensor.to(
...
class VectorizedLocalMap(object):
    ...
    def __init__(self,
                 ...
                 centerline_classes=['centerline'],
                 sample_dist=1,
                 num_samples=250,
                 padding=False,
                 fixed_ptsnum_per_line=-1,
                 oversampled_fixed_ptsnum_per_line=-1,
                 padding_value=-10000,
                 centerline_resolution_meters=2,
                 ordered=False):
        '''
        Args:
        ...
        self.oversampled_fixed_num = oversampled_fixed_ptsnum_per_line
        self.padding_value = padding_value
        self.centerline_resolution_meters = centerline_resolution_meters
        self.ordered=ordered
    ...
    def gen_vectorized_samples(self, location, lidar2global_translation, lidar2global_rotation):
        ...
        for vec_class in self.vec_classes:
            if vec_class == 'divider':
                ...
                for contour in poly_bound_list:
                    ...
                    geom_vectors[vec_class].append(contour)
            # obtain centerlines vectors, in the same format as other elements
            elif vec_class == 'centerline':
                centerline_geom = self.get_map_geom(patch_box, patch_angle, self.centerline_classes, location)
                geom_vectors[vec_class] = []
                centerline_instances_dict = self.line_geoms_to_instances(centerline_geom)
                for line_type, instances in centerline_instances_dict.items():
                    for centerline in instances:
                        vectors.append((centerline, self.CLASS2LABEL.get(line_type, -1)))
                        geom_vectors[vec_class].append(centerline)
            else:
        ...
        gt_instance = LiDARInstanceLines(gt_instance,gt_labels,self.sample_dist,
                        self.num_samples, self.padding, self.fixed_num, self.oversampled_fixed_num,
                        self.padding_value, patch_size=self.patch_size,ordered=self.ordered)
    ...
    def get_map_geom(self, patch_box, patch_angle, layer_names, location):
        ...
        for layer_name in layer_names:
            if layer_name in self.line_classes:
                ...
                map_geom.append((layer_name, geoms))
            elif layer_name in self.centerline_classes:
                geoms = self.get_centerline_line(patch_box, patch_angle, location)
                map_geom.append((layer_name, geoms))
        return map_geom
    ...
    def get_centerline_line(self, patch_box, patch_angle, location):
        '''
        Obtain the centerline vectors in the current sample scene. 
        '''
        patch_x = patch_box[0]
        patch_y = patch_box[1]

        patch = NuScenesMapExplorer.get_patch_coord(patch_box, patch_angle)
        # Since we obtained discretized centerlines, we need some processing steps to
        # merge adjacent centerlines without intersections (merge vertices with degree as 2)
        graph = Graph()
        lines = self.nusc_maps[location].discretize_centerlines(resolution_meters=2)
        for line in lines:
            line = [tuple([round(v[0],3),round(v[1],3)]) for v in line]
            ...
            if not new_line.is_empty:
                ...
                if new_line.geom_type=='MultiLineString':
                    for new_single_line in new_line.geoms:
                        graph.add_e(new_single_line)
                elif new_line.geom_type=='LineString':
                    graph.add_e(new_line)
        # graph merge
        line_list = []
        for _, v in graph.vertices.items():
            # if two centerlines connect and only connect with one another centerline,
            # merge them
...
class CustomNuScenesTIDataset(CustomNuScenesDataset):
    r"""NuScenes Dataset.
    ...
    def __init__(self,
        ...
        self.vector_map = VectorizedLocalMap(kwargs['data_root'], 
                            ...
                            oversampled_fixed_ptsnum_per_line=oversampled_fixed_ptsnum_per_line,
                            padding_value=self.padding_value,
                            ordered=ordered)
        self.is_vis_on_test = False
        self.noise = noise
        self.noise_std = noise_std
        self.val_vis = val_vis
        self.ordered = ordered
        self.temp_epoch_counter = 0
    @classmethod
    ...
    def _evaluate_single(self,
        ...
        cls_gens, cls_gts = format_res_gt_by_classes(result_path,
                                                     ...
                                                     val_vis=self.val_vis,
                                                     temp_epoch_counter=self.temp_epoch_counter)
