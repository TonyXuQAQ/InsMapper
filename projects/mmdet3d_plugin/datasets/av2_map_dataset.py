...
class VectorizedAV2LocalMap(object):
    ...
    def gen_vectorized_samples(self, location, map_elements, lidar2global_translation, lidar2global_rotation):
        ...
        for vec_class in self.vec_classes:
            if vec_class == 'divider':
                ...
                centerline = self.get_centerline_geom(patch_box,patch_angle,map_elements[vec_class], ego_SE3_city)
                centerline_list = self.centerline_geoms_to_instances(centerline)
    ...
    def get_centerline_geom(self, patch_box, patch_angle, avm, ego_SE3_city):
        graph = Graph()
        map_boundary_geom = []
        patch_x = patch_box[0]
        patch_y = patch_box[1]
...
class CustomAV2InsMapDataset(CustomNuScenesDataset):
    r"""NuScenes Dataset.
