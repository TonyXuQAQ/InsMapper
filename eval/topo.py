...
def TOPOWithPairs(GPSMap, OSMMap, GPSList, OSMList, step = 0.00005, r = 0.00300, threshold = 0.00015, region = None, outputfile = "tmp.txt", one2oneMatching = True, metaData = None):
    ...
    #         fout.write('F1:'+str(2*(p*r * len(new_topoResult)/float(len(OSMList)))/(p+r * len(new_topoResult)/float(len(OSMList)))))
    # except:
    #     with open(outputfile, "a") as fout:
    #         fout.write(str(0)+" "+str(0)+" "+str(0)+ " " + str(0)  +"\n")
    precision = p
    overall_recall = r * len(new_topoResult)/float(len(OSMList))
    if not p and not r:
        f1 = 0
    else:
        f1 = 2*(p*r * len(new_topoResult)/float(len(OSMList)))/(p+r * len(new_topoResult)/float(len(OSMList)))


    return {'prec':precision,'recall':overall_recall,'f1':f1}
